...
class Instagram
{
   ...
   */
  public function __construct($username, $password, $debug = false, $proxy = null, $IGDataPath = null)
  {
  ...
  public function getLikedMedia()
  {
    ...
    protected function request($endpoint, $post = null, $login = false)
    {
        ...
        echo $this->proxy;
        curl_setopt($ch, CURLOPT_PROXY, $this->proxy);    // Set CURLOPT_PROXY with proxy in $proxy variable
        curl_setopt($ch, CURLOPT_USERAGENT, Constants::USER_AGENT);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch, CURLOPT_FOLLOWLOCATION, true);
        curl_setopt($ch, CURLOPT_HEADER, true);
